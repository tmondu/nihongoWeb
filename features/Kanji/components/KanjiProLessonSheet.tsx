            ...
            return (
              ...
              >
                ...
                <div className='grid grid-cols-1 md:grid-cols-12'>
                  ...
                  {/* ===================================================== */}
                  <div className='flex flex-col items-center justify-between border-b border-(--border-color) bg-(--card-color) p-5 text-center md:col-span-3 md:border-r md:border-b-0'>
                    {/* Âm Hán-Việt (In hoa, màu hồng san hô / rose) */}
                    <span className='font-sans text-sm font-black tracking-wider text-rose-500 uppercase sm:text-base dark:text-rose-400'>
                      {word.hanviet}
                    </span>

                    {/* Chữ Kanji lớn (Màu xanh lá cây đặc trưng) */}
                    <div className='my-2 flex flex-col items-center'>
                      <span className='font-japanese text-5xl font-black text-emerald-600 transition-transform select-none hover:scale-110 sm:text-6xl dark:text-emerald-400'>
                        {word.kanjiChar}
                      </span>

                      {/* Nút phát âm dạng viên thuốc "🔊 Phát âm" */}
                      <button
                        type='button'
                        onClick={() => handlePlayAudio(word.kanjiChar)}
                        className='mt-2.5 inline-flex cursor-pointer items-center gap-1.5 rounded-full border border-(--border-color) bg-(--background-color) px-3.5 py-1 text-xs font-semibold text-(--secondary-color) shadow-2xs transition-colors hover:border-(--main-color)/40 hover:text-(--main-color)'
                    ...
                    <span className='font-sans text-sm font-bold text-sky-600 capitalize sm:text-base dark:text-sky-400'>
                      {word.meaning}
                    </span>
                  </div>
                  ...
                  {/* ===================================================== */}
                  <div className='flex flex-col justify-between border-b border-(--border-color) bg-(--card-color) md:col-span-4 md:border-r md:border-b-0'>
                    {/* Phần trên: KUN & ON */}
                    <div className='flex flex-1 flex-col justify-center space-y-3 p-5'>
                      {/* Kun row */}
                      <div className='flex items-baseline gap-2'>
                        <span className='shrink-0 font-sans text-xs font-black tracking-wider text-(--main-color) uppercase sm:text-sm'>
                          KUN:
                        </span>
                        <span className='font-japanese text-xs font-bold text-amber-500 sm:text-sm dark:text-amber-400'>
                          {word.kunyomi}
                        </span>
                      </div>

                      {/* Dashed divider */}
                      <div className='border-b border-dashed border-(--border-color)' />
                    ...
                    <div className='flex min-h-[44px] items-center border-t border-dashed border-rose-500/30 bg-rose-500/10 px-4 py-3 sm:px-5 sm:py-3.5'>
                      <span className='font-sans text-xs font-medium text-(--secondary-color) italic'>
                  ...
                  {/* ===================================================== */}
                  <div className='flex flex-col justify-between bg-(--card-color) p-5 md:col-span-5'>
                    <div>
                      <div className='mb-3 flex items-center justify-between'>
                        <h4 className='font-sans text-xs font-black tracking-wider text-(--main-color) uppercase'>
                          VÍ DỤ CÂU & TỪ VỰNG:
                        </h4>
                      ...
                      <div className='space-y-2.5'>
                        {word.examples.map((ex, exIdx) => {
                          return (
                            <div
                              key={exIdx}
                              className='group flex items-center justify-between gap-2.5 rounded-2xl border border-(--border-color) bg-(--background-color) p-2.5 px-3.5 shadow-2xs transition-all hover:border-emerald-500/50'
                            >
                              <div className='flex flex-wrap items-baseline gap-1.5 text-xs sm:text-sm'>
                                <span className='font-sans font-bold text-(--secondary-color)'>
                                  {ex.num}
                                </span>
                                <span className='font-japanese font-bold text-sky-600 dark:text-sky-400'>
                                  {ex.japanese}
                                </span>
                                <span className='font-sans font-medium text-(--main-color)'>
                                  : {ex.meaning}
                                </span>
                              </div>

                              {/* Nút tròn màu xanh lá (CircleDot) nghe âm thanh ví dụ */}
                              <button
                                type='button'
                                onClick={() =>
                                  handlePlayAudio(
                                ...
                                className='flex size-6 shrink-0 cursor-pointer items-center justify-center rounded-full border border-emerald-500/60 bg-emerald-500/10 text-emerald-600 transition-all hover:scale-110 hover:border-emerald-500 hover:bg-emerald-500/20 dark:text-emerald-400'
