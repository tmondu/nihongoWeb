...
const WORKFLOW_FILE = 'thanos-community-content.yml';
 ...
 *
 * Dispatches the Thanos community content workflow.
 *
 * Required env vars:
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error(
      `[trigger-thanos-community-content] GitHub dispatch request failed: ${detail}`,
    );
    return NextResponse.json(
  ...
  if (!response.ok) {
    ...
    console.error(
      `[trigger-thanos-community-content] GitHub API error ${response.status}: ${body}`,
    );
    return NextResponse.json(
      {
